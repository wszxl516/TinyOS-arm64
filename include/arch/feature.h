...
// Provides identification information for the PE, including an implementer code
...
#define GET_CPU_ID()                                                     \
  ({                                                                     \
    ...
    GET_BITS(cpu_id_data, 0, 15) | GET_BITS(cpu_id_data, 16, 23) << 16 | \
        GET_BITS(cpu_id_data, 32, 39) << 24;                             \
  })
