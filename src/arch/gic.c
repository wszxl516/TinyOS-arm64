...
static void init_gicc(void) {
  ...
  /* Note: Higher priority corresponds to a lower Priority field value in the
   * GIC_PMR. In addition to this, writing 255 to the GICC_PMR always sets it to
   * the largest supported priority field value.
   */
...
static void init_gicd(void) {
  ...
  for (i = GIC_INTNO_PPI0 / GIC_GICD_ICFGR_PER_REG;
       (GIC_INT_MAX + (GIC_GICD_ICFGR_PER_REG - 1)) / GIC_GICD_ICFGR_PER_REG >
       i;
       ++i)
...
static i32 gicd_probe_pending(i32 irq) {
  ...
  is_pending =
      (GICD_REGISTER->GIC_GICD_ISPENDR[(irq / GIC_GICD_ISPENDR_PER_REG)] &
...
static void gicd_config(i32 irq, u32 config) {
  ...
  shift = (irq % GIC_GICD_ICFGR_PER_REG) *
          GIC_GICD_ICFGR_SIZE_PER_REG; /* GICD_ICFGR has 16 fields, each field
                                          has 2bits. */
  ...
  reg |= (((u32)config)
          << shift); /* Set the value to the field correponding to irq */
  GICD_REGISTER->GIC_GICD_ICFGR[irq / GIC_GICD_ICFGR_PER_REG] = reg;
}

void gic_eoi(i32 irq) { gicd_clear_pending(irq); }
